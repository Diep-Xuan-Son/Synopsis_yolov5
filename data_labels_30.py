...
def plot(x1, y1, x2, y2, time, obj, frame, classes):
	img0 = cv2.imread('./datasynopsis12/ROI_' + str(frame) +'.jpg')
	xyxy = [x1, y1, x2, y2]
	label = f'{classes} {time}'
	plot_one_box(xyxy, img0, label=label, color=(255, 0, 0) , line_thickness=1)
	cv2.imwrite('./datasynopsis12/ROI_' + str(frame) +'.jpg', img0)

def data_labels_30(t0):
	print("Running...")
	n = 2
	t = t0		#tg cua video 
	...
	for row in cur.execute('SELECT * FROM stocks '):
		if((((float(row[4][0:1])*60 + float(row[4][2:7])) < (t*60/n)) or ((float(row[4][0:1])*60 + float(row[4][2:7])) < (t1 + 4))) and (row[7] == c) ):
			if(row[5] != 0):
				im1 = Image.open('./datasynopsis12/ROI_' + str(row[6]) + '.jpg')
				im3 = Image.open('./datasynopsis10/ROI_' + str(row[6]) + '_' + str(row[5]) + '.jpg')
				im1.paste(im3, (int(row[0]), int(row[1])))
				im1.save('./datasynopsis12/ROI_' + str(row[6]) + '.jpg', quality=95)
				plot(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7])
			else:
				im1 = Image.open('./datasynopsis11/IBG.jpg')
				im2 = Image.open('./datasynopsis10/ROI_' + str(row[6]) + '_0.jpg')
			...
			im1 = Image.open('./datasynopsis12/ROI_' + str(data[i][0]) + '.jpg')
			im2 = Image.open('./datasynopsis10/ROI_' + str(row[6]) + '_' + str(row[5]) + '.jpg')
			w = row[2] - row[0]
			h = row[3] - row[1]
			if((data[i][0] == data[i-1][0]) and (((data[i-1][2] >= data[i][2]) and (data[i-1][2] <= row[0])) or ((data[i-1][2] <= data[i][2]) and (data[i-1][2] >= row[0])) or ((data[i-1][1] >= data[i][1]) and (data[i-1][1] <= row[1])) or ((data[i-1][1] <= data[i][1]) and (data[i-1][1] >= row[1])))):
				...
				if(((row[0] >= data[i][2]) and (row[0] < (data[i][2] + data[i-1][4])) or (row[0] <= data[i][2]) and ((row[0] + w) > data[i][2])) and ((row[1] >= data[i][1]) and (row[1] < (data[i][1] + data[i][3])) or (row[1] <= data[i][1]) and ((row[1] + h) > data[i][1]))):
					im2.putalpha(80)
					im1.paste(im2, (int(row[0]), int(row[1])), im2)
				else:
					im1.paste(im2, (int(row[0]), int(row[1])))
			im1.save('./datasynopsis12/ROI_' + str(data[i][0]) + '.jpg', quality=95)
